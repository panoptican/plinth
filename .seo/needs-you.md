...
| NY-5 | 2026-09-11 | Search Console sitemap | Search Console recognizes `sc-domain:plinth.us`, but the Sitemaps report has zero submitted sitemaps. Submit `https://plinth.us/sitemap.xml` there so Google has an explicit sitemap receipt; this is separate from indexing and should not be treated as proof that URLs are indexed. | Verified in Chrome: `https://plinth.us/sitemap.xml` is submitted, status Success, last read Sep 11, 2026, with 87 pages discovered. | 2026-09-11 |
| NY-6 | 2026-09-11 | internal-link repair deployment | After reviewing the local source changes, will you deploy the internal-link repair so the live health crawl can verify the 66 redirecting `.html` targets have been removed? | Deployed through Wrangler to Cloudflare Pages; the post-deploy crawl reports 0 redirecting internal targets and 0 broken targets across 87 pages. | 2026-09-11 |
| NY-7 | 2026-09-12 | orphan sitemap pages | Should the five sitemap-listed splash routes (`/issue02/splash` through `/issue06/splash`) and the closed `/issue07/submissions` page remain indexable? If yes, approve adding navigation; if no, remove them from the sitemap and add `noindex`. | | |
